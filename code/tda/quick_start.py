...
def generar_circulo_ruidoso(n=200, dim=2, R=1.5, sigma=0.2, seed=42):
    """Círculo de radio R en ℝ^2, embebido en ℝ^dim con ruido gaussiano σ.

    Las dos primeras coordenadas son (R·cos t, R·sin t) + ruido.
    Las coordenadas 3..dim son ruido puro. Modela la maldición de la
    dimensionalidad: al crecer d, las distancias concentran en σ·√(2d).
    """
    rng = np.random.default_rng(seed)
    t = np.linspace(0, 2 * np.pi, n, endpoint=False)
    puntos = np.zeros((n, dim))
    puntos[:, 0] = R * np.cos(t)
    puntos[:, 1] = R * np.sin(t)
    puntos += rng.normal(0, sigma, (n, dim))
    return puntos
...
def ruido_main():
    ...
    print("\n-- Círculo con ruido en distintas dimensiones --")
    for dim in [2, 8, 32, 128]:
        pts = generar_circulo_ruidoso(n=200, dim=dim, R=1.5, sigma=0.2, seed=42)
...
def _null_landscape_95(puntos, lim, eps_range, n_perm=100, dim=0):
    """Calcula el percentil 95 de λ_1 bajo H0 (permutación de coordenadas).

    Cada coordenada se permuta independientemente: destruye la estructura
    topológica (correlaciones entre coordenadas) pero preserva la distribución
    marginal de cada una. El null opera en la misma escala de distancias que
    los datos originales, independientemente de la dimensión ambiente.
    """
    from ripser import ripser as _ripser
    rng_perm = np.random.default_rng(0)
    null_landscapes = np.zeros((n_perm, len(eps_range)))
    for p_idx in range(n_perm):
        pts_null = np.column_stack([
            rng_perm.permutation(puntos[:, j])
...
def fig_invarianza_dimension(out_dir=None):
    """Maldición de la dimensionalidad: círculo R=1.5 embebido en ℝ^d.

    Fila 0: proyección (x_1, x_{d/2}) — señal vs coordenada de ruido.
    Fila 1: proyección PCA a 2D — robusto, recupera el círculo siempre.
    Fila 2: diagrama de persistencia H1 — se debilita con d creciente.
    Fila 3: paisaje λ_1 — cae bajo el umbral bootstrap en alta dimensión.
    """
    plt = _setup_style()
    out_dir = out_dir or DIAGRAMS_DIR
    os.makedirs(out_dir, exist_ok=True)
    ...
    for col, dim in enumerate(dimensiones):
        ...
        h1 = dgms[1]
        h1_fin = h1[np.isfinite(h1[:, 1])] if len(h1) > 0 else h1
        lim = float(h1_fin[:, 1].max()) * 1.15 if len(h1_fin) > 0 else 2.8
        lim = max(lim, 0.5)
        eps_range = np.linspace(0, lim, 300)

        # Fila 0: proyección (x_1, x_{dim//2})
        ax_pts = axes[0, col]
        mid = min(dim // 2, dim - 1)
        ax_pts.scatter(pts[:, 0], pts[:, mid],
                       s=3, c="#2196F3", alpha=0.6, edgecolors="none")
        ax_pts.set_title(
            f"$\\mathbb{{R}}^{{{dim}}}$  $(x_1,x_{{{mid+1}}})$", fontsize=6
        )
        ax_pts.set_xticks([])
        ax_pts.set_yticks([])
        ...
        ax_pca.set_title("PCA 2D", fontsize=7)
        ...
        # Fila 3: landscape H1 (null con misma dimensionalidad)
        ax_land = axes[3, col]
        if len(h1_fin) > 0:
            land = _compute_landscape(h1_fin, eps_range)
            colores_land = ["#D32F2F", "#1976D2", "#388E3C"]
        ...
        if col == 0:
            ax_land.set_ylabel("$\\lambda_k$", fontsize=6)
        ax_land.set_title("Paisaje $H_1$", fontsize=6)
        ax_land.legend(fontsize=4, loc="upper right")
        ax_land.tick_params(labelsize=5)
