...
CACHE_PATH = os.path.join(DATA_DIR, "validacion_nervio_umap_cos_cache.npz")
EPS_COSINE = 0.5
...
def _build_graphs(emb, emb_red):
    ...
    # UMAP-nervio: lente UMAP 2D sobre R^384, clustering coseno en R^384
    lens_umap  = umap_lib.UMAP(n_components=2,  n_neighbors=15,
                                random_state=42).fit_transform(emb)
    graph_umap = mapper.map(
        lens_umap, emb,
        cover=km.Cover(n_cubes=10, perc_overlap=0.4),
        clusterer=DBSCAN(eps=EPS_COSINE, min_samples=3, metric="cosine"),
    )
    ...
    for m in [8, 10, 12]:
        for p in [0.3, 0.4, 0.5]:
            ...
            if nervio_type == "PCA":
                ...
                g = mapper.map(lens, clust,
                               cover=km.Cover(n_cubes=m, perc_overlap=p),
                               clusterer=DBSCAN(eps=EPS_COSINE, min_samples=3,
                                                metric="cosine"))
...
def plot_validacion(results, output_path):
    ...
    NERVIOS = [("PCA", "PCA-nervio\n(lente PCA 2D, clustering PCA 20D)"),
               ("UMAP", "UMAP-nervio\n(lente UMAP 2D, clustering coseno R^384)")]
...
def main(force_recompute=False):
    ...
    print(f"  {emb.shape[0]} papers, embeddings {emb.shape[1]}D")

    print("\nConstruyendo nervios base...")
    graph_pca, graph_umap = _build_graphs(emb, emb_red)
    n_aristas_pca  = sum(len(v) for v in graph_pca["links"].values())
    n_aristas_umap = sum(len(v) for v in graph_umap["links"].values())
